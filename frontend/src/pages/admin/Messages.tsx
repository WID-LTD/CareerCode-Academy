...
import {
  Send, User as UserIcon, Search, Loader2, Trash2, MoreVertical, X,
} from 'lucide-react';
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      {conv.last_message_at && (
                        <span className="text-[10px] text-gray-400 ml-2 flex-shrink-0">{formatTime(conv.last_message_at)}</span>
                      )}
                    </div>
                    <div className="flex items-center justify-between">
          ...
          {activeConversation ? (
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                ...
                {filteredMessages.map(msg => {
                  ...
                  return (
                    <div key={msg.id} className={`group flex ${isMine ? 'justify-end' : 'justify-start'} ${msgSearch ? 'bg-amber-50 dark:bg-amber-900/5 -mx-4 px-4 py-1' : ''}`}>
                      <div className={`relative max-w-[70%] rounded-2xl px-4 py-2 ${isMine ? 'bg-primary-600 text-white rounded-br-none' : 'bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-bl-none'}`}>
                        ...
                        <div className={`flex items-center justify-between gap-2 mt-1 ${isMine ? 'text-primary-100' : 'text-gray-400'}`}>
                          <div className="text-[10px]">{formatTime(msg.created_at)}</div>
                          {isMine && (
                        ...
                        {isMine && (
                          <div className="absolute -top-1.5 -right-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              className="w-6 h-6 rounded-full bg-white dark:bg-gray-700 shadow-md border border-gray-200 dark:border-gray-600 flex items-center justify-center hover:bg-danger-50 dark:hover:bg-danger-900/30 transition-colors"
                            >
                ...
                {isTyping && (
                  <div className="flex justify-start">
                    <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl rounded-bl-none px-4 py-2">
            ...
            <div className="flex-1 flex items-center justify-center text-gray-400">
              <div className="text-center">
                ...
                <p className="text-xs mt-1">Choose a user from the sidebar to start messaging</p>
              </div>
            </div>
          )}
        </div>
