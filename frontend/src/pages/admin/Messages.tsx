...
export default function AdminMessages() {
  ...
  const {
    ...
    markAllAsRead, searchUsers, searchResults, isSearching, clearSearch,
  } = useChatStore();
  ...
  useEffect(() => {
    if (!showNewChat) { setUserSearchQuery(''); clearSearch(); }
  }, [showNewChat]);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-[calc(100vh-8rem)] flex flex-col">
          ...
          {activeConversation && activeUser ? (
            <>
                      ...
                      <motion.div key={msg.id} variants={msgVariants} initial="initial" animate="animate" transition={{ duration: 0.2 }}>
                        ...
                        <div className={`flex ${isMine ? 'justify-end' : 'justify-start'} ${isGrouped ? 'mt-0.5' : 'mt-3'} ${msgSearch ? 'bg-amber-50 dark:bg-amber-900/5 -mx-4 px-4 py-1' : ''}`}>
                          <div className={`relative max-w-[75%] group ${isMine ? 'order-1' : 'order-1'}`}>
                            <div
                              className={`px-4 py-2 text-sm leading-relaxed break-words shadow-sm ${isMine
              ...
              {showScrollBtn && (
                <button
                  ...
                  className="absolute bottom-24 right-8 w-9 h-9 rounded-full bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700 flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors z-10"
                >
            ...
            <div className="flex-1 flex items-center justify-center text-gray-400">
              <div className="text-center px-6">
                ...
                <Button variant="outline" size="sm" className="mt-5" onClick={() => setShowNewChat(true)}>
      ...
      {showNewChat && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowNewChat(false)}>
          <motion.div
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowNewChat(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="max-h-64 overflow-y-auto space-y-1">
              ...
              {searchResults.map((u: any) => (
                ...
                >
                  ...
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full capitalize ${u.role === 'admin' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/20 dark:text-purple-300' : u.role === 'instructor' ? 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/20 dark:text-cyan-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300'}`}>
                    {u.role}
                  </span>
                </button>
              ))}
